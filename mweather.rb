#
# Weather plugin replacement for rbot
...
class MyWeatherPlugin < Plugin
  ...
  def help(plugin, topic="")
    "mweather <STATION> => display the current conditions at the location specified by the STATION code [Lookup your STATION code at http://www.nws.noaa.gov/data/current_obs/ - this will also store the STATION against your nick, so you can later just say \"weather\", weather => display the current weather at the location you last asked for" 
  end
...
plugin.map 'mweather :where', :defaults => {:where => false}
